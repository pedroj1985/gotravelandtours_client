...
function cleanVOFn(order: unknown, pickUpPlace?: unknown, DeliveryPlace?: unknown) {
...
async function activateModal() {
  ...
  if (getValid(iv)) {
    try {
      ...
      await Promise.all(
        data
          ...
          .map(async (item: { Vehiculo: { ProductoId: unknown; Nombre: unknown; CantidadPlazas: unknown; Descripcion: unknown; DescripcionCorta: unknown; TieneSeguro: unknown; TipoTransmision: unknown; ModeloId: unknown; MarcaId: unknown; ProveedorId: unknown }; PrecioOrden: unknown; Distribuidor: { Nombre: unknown; DistribuidorId: unknown } }) => {
    ...
    renderValid(iv, { $refs: { gttTransmision: gttTransmision.value, gttStartDate: gttStartDate.value, gttEndDate: gttEndDate.value }, $children: [] })
  }
}
...
async function loadPickUpPlaces() {
  if (pickUpOpened.value == true) {
    try {
      ...
      let totalResult: unknown[] = []
      data.forEach((item: { Nombre: unknown; id: unknown; PuntoInteresId: unknown }) => {
        totalResult = totalResult.concat({
          nombre: item.Nombre,
          regionid: item.id,
