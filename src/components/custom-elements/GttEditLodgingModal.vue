...
async function searchResultSameItem() {
  ...
  const fr = v.filter(
    (i: any) => !i.l.some((u: any) => u.habitacion.PrecioOrden < 0),
  );
  if (fr.length == 0) {
...
async function searchResult() {
  ...
  if (getValid(iv)) {
    ...
    if (selectedDestiny.value.type == "HTL") {
      ...
      router.push({
        name: "lodging-detail",
        params: { id: selectedDestiny.value.id },
      });
    } else {
      const searchFilters = {
        Destiny: selectedDestiny.value,
        Region: {
          RegionId: selectedDestiny.value.id,
          RegionNombre: selectedDestiny.value.nombre,
        },
        Cliente: { ClienteId: localStorage.getItem("cliente") },
        Entrada: dateIn.value,
        Salida: dateOut.value,
