<template>
    <div id="reserve-cart" class="custom-padding-top">
        ...
        <div class="reserve-content custom-margin" v-if="allTypesOrders.length>0">
            <div class="reserve-cancelation-info">
            </div>
                ...
                <div class="reserve-left-row col-md-3 col-sm-5">
                    <div id="reserve-total-preview" class="pr-30">
                        <div class="background-yellow br-10 pad-15">
                            <div class="reserve-title text-center hn-roman gtt-first-color font24 general-text-opt">
                                Usted ha reservado:
                            </div>
                            <div class="reserve-card" v-for="item in allTypesOrders" :key="item.id">
                                <div class="img-wrapper">
                                    <img :src="item.imagen" :alt="item.nombre">
                                </div>
                                <div class="reserve-card-info pad-5 bg-white">
                                    <div class="reserve-card-item-name hn-roman font14 gtt-text-color">
                                        {{item.nombre}}
                                    </div>
                                    <div class="reserve-card-item-price hn-roman font16 gtt-text-color">
                                        {{ styledPrice(item.precio).intPart}}.<sup>{{ styledPrice(item.precio).decimalPart}}</sup> USD
                                    </div>
                                </div>
                            </div>
                            <div class="reserve-total-to-pay">
                                <span class="total-to-pay-text hn-roman font18 gtt-first-color to-uppercase general-text-opt">Total a pagar</span>
                                <span class="antonio-light gtt-first-color font48">
                                    {{ styledPrice(priceTotal).intPart}}.<sup>{{ styledPrice(priceTotal).decimalPart}}</sup> USD
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="reserve-right-row col">
                    <div class="verify-step">
                        ...
                        <div class="verify-step-content pt-30 pr-15 pl-15 pb-15">
                            <RentReservationView class="rrv" v-for="order in allTypesOrders" :item="order" :key="order.id"></RentReservationView>
                        </div>
                    </div>
                    <div class="create-order-step">
                        <div class="create-order-step-title gtt-first-color general-text-opt hn-bdcn font24 pad-15">
                            <span><i class="mdi mdi-account"></i> Paso 2: </span><span>Datos del o los pasajero(s) y crear orden</span>
        ...
        <div class="no-content font24 hn-roman general-text-opt gtt-text-color text-center" v-else>
    ...
    methods: {
        ...
        async reserve(){
            ...
            listaVehiculosOrden.forEach(vo => {
                vo.NombreCliente = this.constructSpacedVal(this.clientName, this.clienteLastName)
                vo.HoraInicio = this.horaLanding
                vo.HoraFin = this.horaTakeoff
                vo.InformacionLlegada = this.constructSpacedVal(this.aerolineaLanding, this.nvueloLanding, ' - ')
                vo.InformacionSalida = this.constructSpacedVal(this.aerolineaTakeoff, this.nvueloTakeoff, ' - ')
            })
            let orden = {
            ...
            try{
                ...
                this.$toasted.show('Orden creada con éxito. A espera de la administración para su aceptación.' ,{
                    type: 'success'
                })
        ...
        fillReserveInfo(orden){
            ...
            orden.Estado = 'Open' 
            orden.NombreClienteFinal = this.constructSpacedVal(this.clientName, this.clienteLastName)
        ...
        getListaVehiculosOrden(){
            let lvo = this.allTypesOrders.filter(item => {
