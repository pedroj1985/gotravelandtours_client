          ...
          <div class="left-side-map">
            <div class="custom-line-1">
              ...
              <div class="result-search">
                <div
                  class="result-search-text-title antonio-regular"
                >Hemos encontrado en {{filters.Region.RegionNombre}} {{resultList.length}} sitios para alojarse.</div>
                <div class="result-search-text hn-roman">
                  Del {{toMoment(filters.Entrada).locale('es').format('DD MMM YYYY')}} al
...
export default {
  ...
  async created() {
    ...
    if (
      this.filters.Visitantes.adults.value >= this.filters.Visitantes.kids.value
    )
  ...
  data() {
    return {
      ...
      todosTipo: [],
      resultList: [],
      organizedBy: ["Precio (asc)", "Precio (desc)"],
      resultTotal: 0,
      filters: {},
