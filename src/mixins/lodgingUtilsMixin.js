  ...
  methods: {
    ...
    existenCombinacionesEnHabitacion(acomodacion, combinaciones) {
      ...
      while (existe && pos < tam) {
        let acomodacionExiste = this.existeAcomodacion(
          acomodacion[pos],
          combinaciones
        );
      ...
      await Promise.all(
        data.map(async i => {
          if (i.Alojamiento.ReferenciaHotetecId !== null) {
            ...
            //console.info('fullLodging', fullLodging);
            let listadoPrecios = [];
            //let tipo = this.$helpers.habitacionPorCantidadPersonas(this.selectedRoomLayout.adults.value)
            let habitaciones = []
            // await Promise.all(
            //   rooms.data.map(async j => {console.info('rooms', rooms);
